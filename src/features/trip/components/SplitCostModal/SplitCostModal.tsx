...
export default function SplitCostModal({
  totalCost,
  participants,
  onClose,
  onSave,
      ...
      >
        ...
        <div className='p-6 space-y-4 max-h-[60vh] overflow-y-auto no-scrollbar'>
          {participants.map((p) => (
            <div
              key={p.id}
              className='flex items-center gap-4 p-4 rounded-2xl bg-surface-container/20 border border-outline-variant/10 group'
            >
              <img
                src={p.avatar}
                alt={p.name}
